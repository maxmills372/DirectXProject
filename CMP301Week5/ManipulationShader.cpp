...
void ManipulationShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, Light* light, Light* light_2, Camera* camera, float counter, float height, float length,float heightMapSize, float perlin_value)
{
	...
	cameraPtr->heightMapSize = heightMapSize;
	cameraPtr->perlin_value = perlin_value;
