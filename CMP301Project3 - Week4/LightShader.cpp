...
void LightShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, Light* light, Light* light_2, Light* light_3, Light* light_4, Camera* camera)
{
	HRESULT result;
	...
	lightPtr->diffuse[2] = light_3->getDiffuseColour();
	lightPtr->ambient[2] = light_3->getAmbientColour();
	lightPtr->position[2] = XMFLOAT4(light_3->getPosition().x, light_3->getPosition().y, light_3->getPosition().z, 1.0f);

	lightPtr->diffuse[3] = light_4->getDiffuseColour();
	lightPtr->ambient[3] = light_4->getAmbientColour();
	lightPtr->position[3] = XMFLOAT4(light_4->getPosition().x, light_4->getPosition().y, light_4->getPosition().z, 1.0f);
