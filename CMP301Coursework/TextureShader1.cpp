...
void TextureShader::initShader(WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	D3D11_BUFFER_DESC blendTexBufferDesc;
	...
	blendTexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	blendTexBufferDesc.ByteWidth = sizeof(BlendTexBufferType);
	blendTexBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	blendTexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	blendTexBufferDesc.MiscFlags = 0;
	blendTexBufferDesc.StructureByteStride = 0;

	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	renderer->CreateBuffer(&blendTexBufferDesc, NULL, &blendTexBuffer);
...
void TextureShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* texture2, Light* light, Light* light_2, float div)//, ID3D11ShaderResourceView* texture2)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	MatrixBufferType* dataPtr;
	BlendTexBufferType* blendTexPtr;
	...
	// Send test data to pixel shader
	deviceContext->Map(blendTexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	blendTexPtr = (BlendTexBufferType*)mappedResource.pData;
	blendTexPtr->divisionAmount = div;

	deviceContext->Unmap(blendTexBuffer, 0);
	bufferNumber = 0;
	deviceContext->PSSetConstantBuffers(bufferNumber, 1, &blendTexBuffer);
