...
void MySinglePointMesh::initBuffers(ID3D11Device* device)
{
	...
	/*for (int i = 0; i < vertexCount; i++)
	{
		...
		//vertices[i].position = XMFLOAT3(i * 10, 0.0f, 0.0f);
		vertices[i].position = XMFLOAT3(0.0f, 0.0f, 0.0f);
	...
	for (int i = 0; i < vertexCount; i++)
	{
		...
		// Load the vertex array with data.
		vertices[i].position = XMFLOAT3((cos(curve) * size) + test, 0.0f, (sin(curve) * size) + test);
...
void MySinglePointMesh::RandomisePosition(ID3D11Device* device)
{
	...
	//{
		// Load the vertex array with data.
	//	vertices[i].position = XMFLOAT3(i*10, 0.0f, 0.0f);// rand() % 20, rand() % 20, 0.0f);
	//}
	...
	float curve = 1.0f;
	float size = 30.0f;
	//curve = (rand() % 5) *(XM_PI * 2);

	for (int i = 0; i < vertexCount; i++)
	{

		// Load the vertex array with data.
		vertices[i].position = XMFLOAT3((cos(curve) * size) * test, 0.0f, (sin(curve) * size) * test);// rand() % 20, rand() % 20, 0.0f);
		test += 0.005f;
